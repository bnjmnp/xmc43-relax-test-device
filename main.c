...
uint32_t post_object_download_hook(uint16_t index, uint8_t subindex, uint16_t flags) {
  switch (index) {
  case 0x7100: {
    switch (subindex) {
    case 0x01: {
...
void foe_init(void) {
  ...
  static foe_cfg_t config = {
      .buffer_size = FLASH_WRITE_BLOCK_SIZE, /* Buffer size before we flush to
                                                destination */
...
uint32_t foe_write_to_test_bin(foe_file_cfg_t *self, uint8_t *data, size_t length) { return 0; }
